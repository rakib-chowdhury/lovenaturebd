...
class Hotel_booking extends Controller
{
    public function show(Request $r)
    {
        ...
        //$data['hotel_bn'] = Hotel_bn::with('hasCountry')->with('hasHotel')->get();
        $data['countryID']=$data['country'][0]->id;
        if($r->input('submit')==1){            
            $data['countryID']=$r->input('country_name');
        }
        $data['hotel'] = Hotel::where('country_id',$data['countryID'])->get();
        $data['hotel_bn'] = Hotel_bn::where('country_id',$data['countryID'])->get();
        $data['contact_info'] = Contact_info::all();        
        return view('vantage.pages.home.hotel_booking', $data);
    }
    ...
    public function hotel_search(Request $r)
    {
        $data['country'] = Country::all();
        $data['contact_info'] = Contact_info::all();
        $data['company_info'] = Company_info::all();
        ...
        if($r->input('submit')==1){
            $data['hotel'] = Hotel::where('country_id',$r->input('country_name'))->get();
            $data['hotel_bn'] = Hotel_bn::where('country_id',$r->input('country_name'))->get();
