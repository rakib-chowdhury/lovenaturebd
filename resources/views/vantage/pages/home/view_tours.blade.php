...
@section('content')
    <section id="deals" class="deals">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
                    <h1 style="color:#14477B">@if(session('language')=='bn'){{$tour_bn[0]->hasCountry->country_name_bn }}{{' '}}{{'ট্যুর এবং আকর্ষণসমূহ '}}@else{{$tour[0]->hasCountry->country_name }}{{' '}}{{'Tours & Attractions'}}@endif </h1>
                </div>
            </div>
            <div class="row">
                <div class="col-md-2">
                    <div class="left_contant left_float">
                        <div class="left_menu" style=" margin-top:-30px;">
                            <div class="heading"
                                 style="margin-bottom:35px;">@if(session('language')=='en'){{'LoveNatureBD Tours And Attractions'}}@else{{'ট্যুর এবং আকর্ষণসমূহ '}}@endif</div>
                            <ul>
                                @if(session('language')=='en')
                                    @foreach ($country as $u)
                                        <li><a href="{{url('package/view_list')}}/{{$u[0]->country_id}}">
                                                {{$u[0]->hasCountry->country_name}} Tour</a></li>
                                    @endforeach
                                @else
                                    @foreach ($country as $u)
                                        <li><a href="{{url('package/view_list')}}/{{$u[0]->country_id}}">
                                                {{$u[0]->hasCountry->country_name_bn}} ট্যুর</a></li>
                                    @endforeach
                                @endif
                            </ul>
                        </div>
                ...
                <div class="">
                    <div class="col-md-10 right_contant_02 right_float">
                        <div class="h5h"
                             style="padding-top:10px;">@if(session('language')=='bn'){{'প্যাকেজের তালিকা'}}@else{{'Package List'}}@endif</div>
                        @if(session('language') == 'bn')
                            @foreach ($tour_bn as $key=>$row)
                                            ...
                                            <div class="col-md-3">
                                                <div>
                                                    ...
                                                    <a href="../../../../phbooking/pid/4/rid/134/packcode/INS001PD.html">
                                                        <img src="{{url('public')}}/images/tour/{{$row->hasImage->image}}"
                        ...
                        <div class="panel panel-default"
                            ...
                            <div class="panel-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-1">
                                        <a href="{{url('details_tours_attractions')}}/{{$row->id}}">
